...
# TODO: make these objects static
class ObjectManager:
    RegistryLock = RLock()
    OBJECT_REGISTRY = {}
    ...
    def getStopper(cls):
        with cls.RegistryLock:
            stopperEvent = StopperEvent['stopper']
            if stopperEvent is None:
    ...
    def registerObject(cls, obj, asObjName=None):
        ...
        with cls.RegistryLock:
            if not cu.isFrameworkObject(obj):
                raise ex.InheritError(
                    f'Object {obj} does not inherit from FrameworkObject and cannot be added to the Object Manager.')
            ...
            if cls.nameInRegistry(asObjName):  # something else already exists with that name.
                raise ex.NameConflict(
                    f'Object with name {asObjName} already exists in registry, cannot add new instance.')
    ...
    def unregisterObject(cls, obj):
        with cls.RegistryLock:
            if not cu.isFrameworkObject(obj):
                raise ex.InheritError(
                    f'Object {obj} does not inherit from FrameworkObject, cannot unregister from ObjectManager.')
            objName = obj.getName()
            if cls.nameInRegistry(objName):
    ...
    def getRegisteredNames(cls, obj):
        ...
        with cls.RegistryLock:
            registeredNames = list(
                filter(lambda x: cls.OBJECT_REGISTRY['Objects'][x] is obj, cls.OBJECT_REGISTRY['Objects'].keys()))
